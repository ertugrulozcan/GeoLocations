...
namespace GeoLocations.MMDB
{
	internal sealed class Decoder
    {
        ...
        internal Decoder(Buffer database, long pointerBase, bool followPointers = true)
        {
			...
			this._followPointers = followPointers;
			this._listActivatorCreator = new ListActivatorCreator();
			this._dictionaryActivatorCreator = new DictionaryActivatorCreator();
			this._typeActivatorCreator = new TypeActivatorCreator();
        }
        ...
        {
            if (this.Decode(typeof(T), offset, out outOffset, injectables, network) is not T decoded)
            {
        ...
        {
            var type = this.CtrlData(offset, out var size, out offset);
            return this.DecodeByType(expectedType, type, offset, size, out outOffset, injectables, network);
        }

        private ObjectType CtrlData(long offset, out int size, out long outOffset)
        {
            if (offset >= _database.Length)
                throw new InvalidDatabaseException("The DB file's data section contains bad data: pointer larger than the database.");
        ...
        private static bool DecodeBoolean(Type expectedType, int size)
        {
            ...
            return size switch
            {
                0 => false,
                1 => true,
                _ => throw new InvalidDatabaseException("The DB file's data section contains bad data: invalid size of boolean."),
            };
        }
		...
		private double DecodeDouble(Type expectedType, long offset, int size)
        {
            ...
            if (size != 8)
                throw new InvalidDatabaseException("The DB file's data section contains bad data: invalid size of double.");
		...
		private float DecodeFloat(Type expectedType, long offset, int size)
        {
            ...
            if (size != 4)
                throw new InvalidDatabaseException("The DB file's data section contains bad data: invalid size of float.");
        ...
        private object DecodeMapToDictionary(
			Type expectedType, 
			long offset, 
			int size, 
			out long outOffset,
        ...
        private object DecodeMapToType(
            ...
            Network? network)
        {
            var constructor = this._typeActivatorCreator.GetActivator(expectedType);
            ...
            Network? network)
        {
            foreach (var param in constructor.AlwaysCreatedParameters)
            {
				...
				*/
                ...
                var activator = this._typeActivatorCreator.GetActivator(param.ParameterType);
                var constructorParams = activator.DefaultParameters();
                SetInjectables(activator, constructorParams, injectables);
                SetNetwork(activator, constructorParams, network);
                SetAlwaysCreatedParams(activator, constructorParams, injectables, network);
                parameters[param.Position] = activator.Activator(constructorParams);
            }
        }
